...
function normalizeConfigValue(raw: unknown): PluginConfig {
  ...
  const config: PluginConfig = {
    ...
    ignore: {
      paths: normalizeIgnorePaths(
        Array.isArray(input.ignore?.paths) ? input.ignore.paths : []
      ),
      types: normalizeIgnoreTypes(
...
export function resolveWorkspaceRootForPath(
  filePath: string
): vscode.WorkspaceFolder | undefined {
  try {
...
export function readProjectConfigForRoot(
  rootPath: string
): ScopedConfigReadResult {
...
export async function writeScopedProjectConfig(
  config: PluginConfig
): Promise<WriteScopedConfigResult> {
  const scopedRoot = resolveScopedWorkspaceRoot();
...
export function resolveIgnorePathsForFile(
  filePath: string,
...
export function buildWorkspaceRootIgnoreMap(
  fallbackPaths: string[]
): Map<string, string[]> {
...
export function isFilePathIgnored(
  filePath: string,
  ignorePaths: string[]
): boolean {
  const normalizedFilePath = toCanonicalPath(filePath);
  return ignorePaths.some((pattern) =>
    normalizedFilePath.includes(toCanonicalPath(pattern))
  );
}
