...
function setWorkspaceContext(params: {
  roots: string[];
  activeFilePath?: string;
}): void {
  const folders = params.roots.map((root, index) =>
  ...
  workspaceExt.getWorkspaceFolder = (uri) =>
    workspaceState.folders.find((folder) =>
      isFileInFolder(uri.fsPath, folder.uri.fsPath)
    );
...
describe('.confession/config.json 正規化', () => {
  it('normalizeIgnorePaths 與 normalizeIgnoreTypes 應去重與去空白', () => {
    expect(
      normalizeIgnorePaths([' dist ', 'src/generated', 'dist', ''])
    ).toEqual(['dist', 'src/generated']);
    expect(
      normalizeIgnoreTypes([' xss ', 'xss', '', 'hardcoded_secret'])
  ...
  it('readScopedProjectConfig 應保留 minimax-cn provider', () => {
    ...
    setWorkspaceContext({
      roots: [root],
      activeFilePath: path.join(root, 'src/index.ts'),
    });
...
describe('.confession/config.json 作用域行為', () => {
  ...
  it('writeScopedIgnoreFile 應以 .confession/config.json 覆寫 ignore.paths 並保留 ignore.types', async () => {
    ...
    const result = await writeScopedIgnoreFile([
      'dist',
      'dist',
      'src/generated',
    ]);
...
describe('root-aware 忽略路徑解析', () => {
  it('resolveIgnorePathsForFile：有 config 時用 config，缺檔時回退 settings', () => {
    ...
    expect(
      resolveIgnorePathsForFile(path.join(rootA, 'src/a.ts'), ['fallback'])
    ).toEqual(['packages/a']);
    expect(
      resolveIgnorePathsForFile(path.join(rootB, 'src/b.ts'), ['fallback'])
    ).toEqual(['fallback']);
  });
  ...
  it('buildWorkspaceRootIgnoreMap + resolveIgnorePathsFromRootMap 應避免跨 root 汙染', () => {
    ...
    expect(
      resolveIgnorePathsFromRootMap(path.join(rootA, 'src/a.ts'), rootMap, [
        'fallback',
      ])
    ).toEqual(['packages/a']);
    expect(
      resolveIgnorePathsFromRootMap(path.join(rootB, 'src/b.ts'), rootMap, [
        'fallback',
      ])
    ).toEqual(['packages/b']);
  });
  ...
  it('isFilePathIgnored 應以字串包含規則判定，並相容斜線差異', () => {
    expect(
      isFilePathIgnored('/repo/src/generated/file.ts', ['src/generated'])
    ).toBe(true);
    expect(isFilePathIgnored('C:\\repo\\dist\\main.js', ['dist/'])).toBe(true);
    expect(isFilePathIgnored('/repo/src/main.ts', ['src/generated'])).toBe(
      false
    );
  });
});
