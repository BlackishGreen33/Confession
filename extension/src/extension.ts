...
function getPluginConfig(): PluginConfig {
  ...
  const settingsConfig: PluginConfig = {
    llm: {
      provider: config.get<'gemini' | 'nvidia' | 'minimax-cn'>(
        'llm.provider',
        'nvidia'
      ),
    ...
    analysis: {
      triggerMode: config.get<'onSave' | 'manual'>(
        'analysis.triggerMode',
        'onSave'
      ),
      depth: config.get<'quick' | 'standard' | 'deep'>(
        'analysis.depth',
        'standard'
      ),
...
export function activate(context: vscode.ExtensionContext) {
  ...
  outputChannel.appendLine(
    `API 模式: ${pluginConfig.api.mode} (${pluginConfig.api.baseUrl})`
  );
  outputChannel.appendLine(
  ...
  async function focusConfessionView(
    view: 'dashboard' | 'vulnerabilities'
  ): Promise<void> {
    // 先確保 Confession 視圖容器被打開，再聚焦指定 view。
    try {
      await vscode.commands.executeCommand(
        'workbench.view.extension.confession-security'
      );
    } catch {
  ...
  const handleConfigFileEvent =
    (eventName: 'create' | 'change' | 'delete') => (uri: vscode.Uri) => {
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration((e) => {
      if (e.affectsConfiguration('confession')) {
        ...
        outputChannel.appendLine(
          `配置已更新: API=${updated.api.baseUrl}, 觸發=${updated.analysis.triggerMode}`
        );
...
async function scanCurrentFile(getConfig: () => PluginConfig): Promise<void> {
  ...
  const beforeVulns = await fetchFileVulnerabilities(
    baseUrl,
    doc.fileName
  ).catch(() => []);
  ...
  try {
    ...
    const allOpenVulns = await fetchAllOpenVulnerabilities(baseUrl).catch(
      () => null
    );
    ...
    if (taskId && isScanTimeoutError(err)) {
      try {
        ...
        const cancelMsg =
          cancelErr instanceof Error ? cancelErr.message : '未知錯誤';
...
async function scanWorkspaceFiles(
  ...
  outputChannel.appendLine(
    `掃描工作區: ${folders.map((f) => f.name).join(', ')}`
  );
  setAnalyzing();
  sendScanProgress('running', 0);
  ...
  try {
    ...
    const candidateUris = uris.filter((uri) => {
      const ignorePaths = resolveIgnorePathsFromRootMap(
        uri.fsPath,
        ignorePathsByRoot,
        config.ignore.paths
      );
    ...
    outputChannel.appendLine(
      `工作區掃描完成，本次新增 ${newCount} 個漏洞，目前開放 ${allVulns.length} 個`
    );
...
/** 忽略漏洞 */
async function handleIgnoreVulnerability(
  vulnId: string,
  getConfig: () => PluginConfig
): Promise<boolean> {
  const config = getConfig();
