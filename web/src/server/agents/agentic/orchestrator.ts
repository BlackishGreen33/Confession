 ...
 */
export async function orchestrateAgentic(
  request: ScanRequest,
  ...
  const changedFiles = request.forceRescan
    ? request.files
    : filterChangedFiles(request.files);
  options.assertNotCanceled?.();
  await notifyFilteredFiles(
    options.onFilteredFiles,
  ...
  const [goPoints, jstsPoints] = await Promise.all([
    go.length > 0
      ? analyzeGoFiles(
          go.map((file) => ({ path: file.path, content: file.content }))
        )
      : [],
    jsts.length > 0
      ? analyzeJsTsFiles(
    ...
    const outcomes = await Promise.all(
      batch.map(async (file) => {
        ...
        try {
          ...
          llmStats.skippedByPolicy += Math.max(
            0,
            filePoints.length - bundle.hotspots.length
          );
...
function rememberFirstFailureMessage(stats: LlmUsageStats, err: unknown): void {
  if (
    typeof stats.lastErrorMessage === 'string' &&
    stats.lastErrorMessage.length > 0
  ) {
    return;
  }
