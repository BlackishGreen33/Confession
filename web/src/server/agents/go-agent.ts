import { analyzeGoWasm } from '@server/analyzers/go-wasm'
...
/** 單一 Go 檔案輸入 */
export interface GoFileInput {
  path: string
  content: string
}

/**
 * Go Agent：對一組 Go 檔案執行 WASM AST 靜態分析 + 關鍵詞掃描，
 * 合併去重後回傳 InteractionPoint[]。
 */
export async function analyzeGoFiles(files: GoFileInput[]): Promise<InteractionPoint[]> {
  const results: InteractionPoint[] = []

  for (const file of files) {
    // Go WASM AST 靜態分析
    const astPoints = await analyzeGoWasm(file.path, file.content)

    // 關鍵詞掃描
    const keywordHits = scanKeywords(file.content)
    const keywordPoints = keywordHitsToInteractionPoints(keywordHits, file.path, 'go')
