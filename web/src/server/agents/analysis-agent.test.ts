...
function buildFileMap(entries: Array<{ path: string; content: string; language: string }>): FileContentMap {
...
describe('analysis-agent', () => {
  ...
  it('standard 模式同檔案多點僅發送一次 LLM 請求', async () => {
    ...
    const points = [
      ...
      buildPoint({ filePath, line: 30, type: 'dangerous_call', patternName: 'eval', confidence: 'high' }),
    ]
    const fileMap = buildFileMap([
      { path: filePath, content: 'line1\nline2\nline3\nline4\nline5', language: 'typescript' },
    ])
  ...
  it('deep + includeMacroScan 會對每個檔案各呼叫一次且保留全檔', async () => {
    const fileMap = buildFileMap([
      { path: '/workspace/a.ts', content: 'eval(code)', language: 'typescript' },
      { path: '/workspace/b.ts', content: 'const safe = true', language: 'typescript' },
    ])
    const points = [
      buildPoint({
  ...
  it('standard 會限制每檔點位上限為 14', async () => {
    ...
    const fileMap = buildFileMap([
      {
        path: filePath,
        content: Array.from({ length: 60 }, (_, index) => `line-${index + 1}`).join('\n'),
        language: 'typescript',
      },
    ])
