...
vi.mock('@server/agents/orchestrator', () => ({ orchestrate: mockOrchestrate }))
...
describe('Scan routes', () => {
  ...
  beforeEach(() => {
    ...
    mockPrisma.scanTask.update.mockImplementation(
      async ({ where, data }: { where: { id: string }; data: Partial<TaskRecord> }): Promise<TaskRecord> => {
        const current = taskState.get(where.id) ?? createDefaultTask({ id: where.id })
    ...
    mockPrisma.scanTask.findUnique.mockImplementation(async ({ where }: { where: { id: string } }) => {
    ...
    mockPrisma.scanTask.findFirst.mockImplementation(async () => {
      ...
      return tasks.sort((a, b) => b.updatedAt.getTime() - a.updatedAt.getTime())[0] ?? null
    })
  ...
  it('POST /api/scan 未傳 engineMode 時預設使用 agentic_beta', async () => {
    const res = await app.request('/api/scan', {
      ...
      body: JSON.stringify({
        files: [{ path: '/repo/a.ts', content: 'const a = 1', language: 'typescript' }],
        depth: 'standard',
        scanScope: 'file',
      }),
    })
    ...
    const createArg = mockPrisma.scanTask.create.mock.calls[0]?.[0] as { data: TaskRecord }
  ...
  it('POST /api/scan 發起新任務時會中止既有 running 任務', async () => {
    ...
    const res = await app.request('/api/scan', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        files: [{ path: '/repo/new.ts', content: 'const b = 2', language: 'typescript' }],
        depth: 'standard',
      }),
    })
    expect(res.status).toBe(201)
    ...
    expect(oldTaskAfter?.errorMessage).toBe('新掃描任務已啟動，上一個掃描已中止')
  })
  ...
  it('agentic 失敗時會自動回退 baseline，最終 completed + fallbackUsed=true', async () => {
    ...
    const finalTask = await waitForTaskStatus(taskState, created.taskId, 'completed')
  ...
  it('agentic 與 baseline 都失敗時，任務 failed 且 errorCode=BETA_ENGINE_FAILED', async () => {
    ...
    const created = (await res.json()) as { taskId: string }
    const finalTask = await waitForTaskStatus(taskState, created.taskId, 'failed')
  ...
  it('workspace 掃描完成後會自動關閉不在快照內的 open 漏洞', async () => {
    ...
    const res = await app.request('/api/scan', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        files: [{ path: '/repo/active.ts', content: 'const a = 1', language: 'typescript' }],
        depth: 'standard',
        scanScope: 'workspace',
