...
type ExportFormat = z.infer<typeof exportBodySchema>['format']
type ExportFilters = z.infer<typeof exportBodySchema>['filters']

interface SerializedVulnerability {
  id: string
  filePath: string
  line: number
  column: number
...
function buildWhere(filters: ExportFilters): Record<string, unknown> {
  ...
  if (filters.search) {
    where.OR = [
      ...
      { filePath: { contains: filters.search } },
      { type: { contains: filters.search } },
      { cweId: { contains: filters.search } },
    ]
  }
...
function serializeVuln(v: {
  ...
  return {
    ...
    endColumn: Number(v.endColumn),
    codeSnippet: String(v.codeSnippet),
    codeHash: String(v.codeHash),
    type: String(v.type),
    cweId: normalizeNullableString(v.cweId),
    severity: String(v.severity),
    description: String(v.description),
    riskDescription: normalizeNullableString(v.riskDescription),
    fixOldCode: normalizeNullableString(v.fixOldCode),
    fixNewCode: normalizeNullableString(v.fixNewCode),
    fixExplanation: normalizeNullableString(v.fixExplanation),
    ...
    owaspCategory: normalizeNullableString(v.owaspCategory),
...
function buildSummary(items: SerializedVulnerability[]): ExportSummary {
  const bySeverity: Record<string, number> = {}
  const byStatus: Record<string, number> = {}
  const byHumanStatus: Record<string, number> = {}
  const byType: Record<string, number> = {}

  for (const item of items) {
    bySeverity[item.severity] = (bySeverity[item.severity] ?? 0) + 1
    byStatus[item.status] = (byStatus[item.status] ?? 0) + 1
    byHumanStatus[item.humanStatus] = (byHumanStatus[item.humanStatus] ?? 0) + 1
...
function buildReportV2(items: SerializedVulnerability[], filters: ExportFilters): ExportReportV2 {
  return {
    schemaVersion: REPORT_SCHEMA_VERSION,
    reportName: 'Confession Vulnerability Report',
    generatedBy: 'Confession',
...
function renderMarkdown(report: ExportReportV2): string {
...
function renderPrintableHtml(report: ExportReportV2): string {
...
function groupBySeverity(items: SerializedVulnerability[]) {
