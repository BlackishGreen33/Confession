...
export function computeRemediationScore(
  ...
  const closureRate =
    recentDetected.length > 0
      ? recentDetected.filter((item) => item.status === 'fixed').length /
        recentDetected.length
      : 1
...
export function computeReliabilityScore(tasks: HealthScoreInputTask[]): {
  value: number
  successRate: number
  fallbackRate: number
  ...
  const latencyPenalty =
    Math.max(0, workspaceP95Ms - LATENCY_TARGET_MS) / LATENCY_TARGET_MS
  const sLatency = 100 * Math.exp(-latencyPenalty)

  const value =
    0.5 * (successRate * 100) +
    0.2 * ((1 - fallbackRate) * 100) +
    0.3 * sLatency
  return {
    value: clamp(value, 0, 100),
...
export function geometricWeightedScore(values: {
  exposure: number
  remediation: number
  quality: number
  reliability: number
...
export function toHealthGrade(score: number): HealthGrade {
...
export function buildTopFactors(source: TopFactorSource): HealthTopFactor[] {
  ...
  const latencyPenalty =
    Math.max(0, source.reliability.workspaceP95Ms - LATENCY_TARGET_MS) /
    LATENCY_TARGET_MS
...
function buildFactorCandidate(params: {
  ...
  return {
    ...
    reason:
      direction === 'positive'
        ? params.reasonWhenPositive
        : params.reasonWhenNegative,
    impactScore,
  }
}
