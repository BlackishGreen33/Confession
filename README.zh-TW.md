...
- **雙引擎掃描**：預設使用 `agentic_beta`，失敗時同一 task 內自動回退 `baseline`。
...
> [!NOTE]
> 預設掃描引擎是 `agentic_beta`，若失敗後端會在同一個 task 內自動回退 `baseline`。`pdf` 匯出實際回傳的是列印版 HTML，需要透過瀏覽器或 Webview 列印流程另存為 PDF。
